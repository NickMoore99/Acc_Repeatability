...
label.grid(column=2, row=72, columnspan=12)
...
def get_data():
	...
	with f_in:
		...
		for row in reader:
			#print("row: ", row)
			if num_rows2 > 0:
				T.append(float(row[18]))
				X.append(float(row[24]))
				Y.append(float(row[30]))
			num_rows2 += 1		
	f_in.close()
	uT = np.mean(T)
	sT = np.std(T)
	C_Theta = [e for e in T if (uT - 6 * sT < e < uT + 6 * sT)]
	uX = np.mean(X)
	sX = np.std(X)
	X_axis = [e for e in X if (uX - 6 * sX < e < uX + 6 * sX)]
	uY = np.mean(Y)
	sY = np.std(Y)
	Y_axis = [e for e in Y if (uY - 6 * sY < e < uY + 6 * sY)] 
    	
	Tnum_rows = 0	
	while Tnum_rows < len(C_Theta):
		Tnum_rows += 1
		T_Terms.append(Tnum_rows)
	Xnum_rows = 0
	while Xnum_rows < len(X_axis):
		Xnum_rows += 1
		X_Terms.append(Xnum_rows)
	Ynum_rows = 0
	while Ynum_rows < len(Y_axis):
		Ynum_rows += 1
		Y_Terms.append(Ynum_rows)
...
def animate(i, x1s, x2s, y1s):
	...
	X_axis_Std = np.std(X_axis,dtype = np.float64)
	Y_axis_Avg = np.mean(Y_axis)
	Y_axis_Std = np.std(Y_axis)
	
	
	
	ax1.scatter(T_Terms,C_Theta, s = (2)**2, c='g')
	ax2.scatter(X_Terms,X_axis, s = (2)**2,  c='b')
	ax3.scatter(Y_Terms,Y_axis, s = (2)**2, c='r')

	ax1.hlines(y = C_Theta_Avg, xmin = 0, xmax = T_Terms[Tnum_rows-1], colors = 'g', linestyles = 'dashed')
	ax1.hlines(y = C_Theta_Avg+6*Theta_Std, xmin = 0, xmax = T_Terms[Tnum_rows-1],linestyles = 'dashdot')
	ax1.hlines(y = C_Theta_Avg-6*Theta_Std, xmin = 0, xmax = T_Terms[Tnum_rows-1],linestyles = 'dashdot')

	ax2.hlines(y = X_axis_Avg, xmin = 0, xmax = X_Terms[Xnum_rows-1], colors = 'b', linestyles = 'dashed')
	ax2.hlines(y = X_axis_Avg+6*X_axis_Std, xmin = 0, xmax = X_Terms[Xnum_rows-1],linestyles = 'dashdot')
	ax2.hlines(y = X_axis_Avg-6*X_axis_Std, xmin = 0, xmax = X_Terms[Xnum_rows-1],linestyles = 'dashdot')

	ax3.hlines(y = Y_axis_Avg, xmin = 0, xmax = Y_Terms[Ynum_rows-1], colors = 'r', linestyles = 'dashed')
	ax3.hlines(y = Y_axis_Avg+6*Y_axis_Std, xmin = 0, xmax = Y_Terms[Ynum_rows-1],linestyles = 'dashdot')
	ax3.hlines(y = Y_axis_Avg-6*Y_axis_Std, xmin = 0, xmax = Y_Terms[Ynum_rows-1],linestyles = 'dashdot')
